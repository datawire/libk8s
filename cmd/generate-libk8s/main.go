...
func getGodep(repo, commit string) (godep.Godep, error) {
	url := "https://raw.githubusercontent.com/kubernetes/" + repo + "/" + commit + "/Godeps/Godeps.json"
	resp, err := http.Get(url)
	if err != nil {
		return godep.Godep{}, fmt.Errorf("%v %v", url, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return godep.Godep{}, fmt.Errorf("%v HTTP %v", url, resp.StatusCode)
	}
...
func Main(arg0, version string) error {
	...
	startGodep, err := getGodep(StartRepo, commit)
	if err != nil {
		return err
	}

	godeps := []godep.Godep{startGodep}
	for _, dep := range startGodep.Deps {
